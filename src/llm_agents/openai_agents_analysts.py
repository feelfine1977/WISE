...
class PDF(FPDF):
    def __init__(self):
        ...
        self.add_font('DejaVu', '', f'{path}/fonts/DejaVuSans.ttf', uni=True)
        self.add_font('DejaVu', 'B', f'{path}/data/fonts/DejaVuSans-Bold.ttf', uni=True)
...
def main(summary_needed=True,categories=None, folder_path=None,api_key=None):
    if not folder_path:
        settings = load_settings()
        path = settings.get('data_path')
        folder_path = f'{path}results/data_BPIC_2019/'
    else:
        folder_path = folder_path
    output_path = f'{folder_path}/analysis_report_BPIC_2019.pdf'
